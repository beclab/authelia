...
func ResetPasswordPOST(ctx *middlewares.AutheliaCtx) {
	...
	// Send Notification.
	userInfo, err := ctx.Providers.UserProvider.GetDetails(username, userSession.AccessToken)
	if err != nil {
	...
	ctx.Logger.Debugf("Sending an email to user %s to inform that the password has changed.",
		username)
