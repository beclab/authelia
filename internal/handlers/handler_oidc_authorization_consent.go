...
func handleOIDCAuthorizationConsent(ctx *middlewares.AutheliaCtx, issuer *url.URL, client *oidc.Client,
	...
	case client.IsAuthenticationLevelSufficient(userSession.AuthenticationLevel):
		if subject, err = ctx.Providers.OpenIDConnect.GetSubject(ctx, client.GetSectorIdentifier(), userSession.Username); err != nil {
			...
			ctx.Providers.OpenIDConnect.WriteAuthorizeError(ctx, rw, &terminusAuthorizeRequester{requester.(*fosite.AuthorizeRequest), r}, oidc.ErrSubjectCouldNotLookup)
		...
		default:
			...
			ctx.Providers.OpenIDConnect.WriteAuthorizeError(ctx, rw, &terminusAuthorizeRequester{requester.(*fosite.AuthorizeRequest), r}, fosite.ErrServerError.WithHint("Could not determine the client consent mode."))
