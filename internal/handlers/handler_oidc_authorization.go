...
func OpenIDConnectAuthorization(ctx *middlewares.AutheliaCtx, rw http.ResponseWriter, r *http.Request) {
	...
	if requester, err = ctx.Providers.OpenIDConnect.NewAuthorizeRequest(ctx, r); err != nil {
		...
		ctx.Providers.OpenIDConnect.WriteAuthorizeError(ctx, rw, &terminusAuthorizeRequester{requester.(*fosite.AuthorizeRequest), r}, err)
	...
	if userSession, err = ctx.GetSession(); err != nil {
		...
		ctx.Providers.OpenIDConnect.WriteAuthorizeError(ctx, rw, &terminusAuthorizeRequester{requester.(*fosite.AuthorizeRequest), r}, fosite.ErrServerError.WithHint("Could not obtain the user session."))
	...
	if authTime, err = userSession.AuthenticatedTime(client.Policy); err != nil {
		...
		ctx.Providers.OpenIDConnect.WriteAuthorizeError(ctx, rw, &terminusAuthorizeRequester{requester.(*fosite.AuthorizeRequest), r}, fosite.ErrServerError.WithHint("Could not obtain the authentication time."))
	...
	ctx.Providers.OpenIDConnect.WriteAuthorizeResponse(ctx, rw, &terminusAuthorizeRequester{requester.(*fosite.AuthorizeRequest), r}, responder)
}
